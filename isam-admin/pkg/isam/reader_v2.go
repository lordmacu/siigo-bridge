...
func parseV2Header(data []byte) (*V2Header, error) {
	...
	case data[0] == 0x33 && data[1] == 0xFE:
		// MF IDXFORMAT 8 single-file format (data + B-tree index interleaved)
		// Uses stream record type markers (2-byte) with 8-byte alignment
		// NOT the old "indexed marker" byte-scan format
		h.LongRecords = false
		h.IsIndexed = false // use stream scanner (type nibbles + alignment)
	...
	if h.IsIndexed {
		// Legacy indexed marker format (NOT 0x33FE — see above)
		if len(data) < 0x44 {
		...
		h.RecHeaderSize = 2
		h.Alignment = 0
	} else if h.Magic>>16 == 0x33FE {
		// 0x33FE: MF IDXFORMAT 8 single-file indexed format
		// Header layout differs from 0x307E: record size at different offset
		h.HeaderSize = 128

		// Timestamps (14 chars each, same position as standard)
		h.CreationDate = string(data[8:22])
		h.ModifiedDate = string(data[22:36])

		// 0x33FE-specific field positions
		h.Organization = 2 // always indexed
		h.IdxFormat = 8    // always IDXFORMAT 8
		h.RecHeaderSize = 2
		h.Alignment = 8

		// Record size at 0x38 (2 bytes, big-endian)
		h.MaxRecordLen = uint32(binary.BigEndian.Uint16(data[0x38:0x3A]))
		h.MinRecordLen = h.MaxRecordLen
	} else {
		// MF stream format (0x307E and variants): standard 128-byte header layout
		h.HeaderSize = 128
		...
		// Variant 0x30xx indexed files (e.g., Z06: 0x300003FC) use 0x800 header
		// The 128-byte header is followed by index metadata up to 0x800;
		// actual record stream starts at 0x800
		isStandardMagic := (data[0] == 0x30 && data[1] == 0x7E) ||
			(data[0] == 0x30 && data[1] == 0x00 && data[2] == 0x00 && data[3] == 0x7C)
		if !isStandardMagic && h.Organization == 2 {
			h.HeaderSize = 0x800
		}
	}
